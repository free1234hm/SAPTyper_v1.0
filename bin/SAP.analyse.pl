...
if ($par_term2value_hash{'db_searching'}==1)  #第二个模块
{
	...
	elsif ($DS_search_engine eq 'MaxQuant')
	{
		...
		$search_result_tmp=$search_result_dir.'\result\combined\txt\evidence.txt';
		$search_result_tmp=~s/\\\\/\\/g;
	}
}
if ($par_term2value_hash{'data_ana'}==1)  #第三个模块
{
	if ($par_term2value_hash{'db_creation'}==1)  #有第一个模块
	{
		...
		$SAP_anno="$current_path".'\result\all.anno.singleSAP.txt';  #202504：注释文件固定，不再按以下变量调整，并没有要采用所有的注释的情况。但var_type1的判断目前没用到
...
#		}
	...
	else
	{
		...
		$search_engine_id=$DA_search_engine_id;
	}
...
';
	...
	while ($in_vcf=<IN_vcf>)
	{
		...
		$in_vcf=~s/\t[ ]+/\t/g;   #20250412修改，为了避免因为末端空格带来的问题
		$in_vcf=~s/[ ]+$//;
...
';
	...
	while ($in_pro=<IN_pro>)
	{
		...
		$in_pro=~s/\t[ ]+/\t/g;   #20250412修改，为了避免因为末端空格带来的问题
		$in_pro=~s/[ ]+$//;
